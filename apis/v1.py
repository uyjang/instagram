...
class UserLoginView(BaseView):
    def post(self, request):
        ...
        user = authenticate(request, username=username, password=password) # authenticate는 장고가 기본적으로 만들어 놓은 것으로 username이나 password를 잘못 입력했을 때 None을 반환해주는 기능을 가짐
        if user is None:
            return self.response(message='입력 정보를 확인해주세요.', status=400)
        login(request, user) # 장고가 기본적으로 만들어 놓은 기능
...
class UserCreateView(BaseView):
    def post(self, request): # if request=='POST' 할 때 그 post
        # Restful 하게 하기 위해선 POST, DELETE 다 나누어야 하지만, 편의상 POST만 사용하도록 하겠다.
        username = request.POST.get('username', '')
        if not username:
            return self.response(message='아이디를 입력해주세요.', status=400) # response는 BaseView에 있는 reponse를 뜻한다.
        password = request.POST.get('password', '')
        if not password:
            return self.response(message='패스워드를 입력해주세요.', status=400)
...
class ContentCreateView(BaseView): # 인스타그램에 포스팅하는 작업을 하는 기능
...
class UserGetView(BaseView):
    ...
    def get(self, request): # 유저네임을 입력해서 조회만 할 것이니 get만 있으면 됨
